#!/bin/bash -l
BASE_DIR="/home/$USER/DGMS-master/src/glue"
...
sbatch --time=04:00:00 --nodes=1 --gpus-per-node=1 --mem=80G --cpus-per-task=16 --constraint="a100-80gb" <<EOT
#!/bin/bash -l

#SBATCH --job-name ${MODEL_NAME}-${METHOD}
#SBATCH --output /home/$USER/DGMS-master/out/%x_%j.out
#SBATCH --error /home/$USER/DGMS-master/err/%x_%j.err
...
nvidia-smi
python ${BASE_DIR}/glue_training.py $DATASET --task_name ${TASK_NAME} --K ${K} --prune --tau ${TEMP} --weight_decay ${WD} --model_name ${MODEL_NAME} \
       ...
       --eval_batch_size ${EVAL_BATCH_SIZE} --save_folder /scratch/gilbreth/$USER/DGMS/Run/GLUE_sst/compressed_${MODEL_NAME}/${METHOD}/ 
