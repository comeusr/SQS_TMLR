...
nvidia-smi
python /home/$USER/DGMS-master/src/glue/glue_training.py $DATASET --task_name ${TASK_NAME} --K ${K} --tau ${TEMP} --weight_decay ${WD} --prune --model_name ${MODEL_NAME} \
       ...
       --batch_size ${BATCH_SIZE} --max_seq_length ${MAX_LENGTH} --preprocessing_num_workers 4 --method ${METHOD} \
       --eval_batch_size ${EVAL_BATCH_SIZE} --save_folder /scratch/gilbreth/$USER/DGMS/Run/GLUE/compressed_${MODEL_NAME}/
EOT